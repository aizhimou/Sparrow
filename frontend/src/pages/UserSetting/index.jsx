...
const UserSetting = () => {
  ...
  const resetPassword = async (values) => {
    ...
    if (code === 200) {
      showSuccess(
          'Password reset successfully! Use new password to login next time.');
    } else {
      showError(msg);
    }
  ...
  const sendVerificationCode = async () => {
    ...
    const res = await API.post(
        '/api/account/sendVerificationEmail', user
    );
    const {code, msg} = res.data;
    if (code === 200) {
      showSuccess(
        ...
        <Stack mt='lg'>
          ...
          <form onSubmit={resetPasswordForm.onSubmit(
            ...
            <Button mt='sm' loading={resetPasswordLoading} type='submit'
                    fullWidth>Confirm Reset</Button>
          </form>
        </Stack>
        <Divider/>
        <Stack mt='md'>
          ...
          <form
            ...
            <Group align='flex-end'>
              <TextInput name='email'
                     ...
                     style={{flex: 1}}
              />
              <Button variant="outline" loading={getCodeLoading}
                      onClick={sendVerificationCode}>Get Code</Button>
            </Group>
            <TextInput
                mt='sm'
