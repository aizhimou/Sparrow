...
import {
  ...
  Button,
  Badge,
  Input, Title, Modal, TextInput, PasswordInput, Select
} from "@mantine/core";
...
const User = () => {
  ...
  const fetchUsers = async () => {
    const res = await API.get(
        `/api/user/list?page=${page}&size=${PAGE_SIZE}&keyword=${keyword}`);
      ...
      <Container size="lg" mt="lg">
        <Group mt='lg'>
          <Input
              ...
              style={{flex: 1}}
          />
          <Button leftSection={<IconSearch size={16}/>} onClick={fetchUsers} color="blue">Search</Button>
          <Button leftSection={<IconPlus size={16}/>} onClick={open} color="cyan">New User</Button>
        </Group>
        <DataTable
            mt='md'
            verticalSpacing="sm"
            ...
            columns={[
              ...
              {accessor: 'email', title: 'Email'},
              {
                ...
                render: (user) =>
                    <Badge radius="xs" variant='light'
                           color={user.role < 0 ? 'red' : user.role === 0
                               ? 'orange' : 'green'}>
              ...
              },
              {
                ...
                render: (user) =>
                    <Badge radius="xs" variant="light"
                           color={user.status === 1 ? 'green' : 'gray'}>
              ...
              },
              {
                accessor: 'createdAt',
                title: 'Created At',
                render: ({createdAt}) => dayjs(createdAt).format(
                    'YYYY-MM-DD HH:mm'),
              },
              {
                accessor: 'actions',
